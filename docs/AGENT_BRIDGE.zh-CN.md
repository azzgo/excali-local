...
新装默认下桥接主开关为**开启**（画布上的 Agent 按钮开箱即见）；配对与按画布激活仍需你显式授权。最快的路径是在画布上做一次操作；Options 页面是一个保守的总开关。
...
> 在那里把它打开会启用该功能，但*不会*自动配对或激活——请使用画布按钮。当 **"隐藏 AI 按钮"**
> 开启时，打开主开关会被拒绝（并有提示）——需先显示按钮：没有可见的按钮就没有可配对/激活的
> 画布控件，页面不会静默替你做出选择。把它**关掉**会立即拆除
