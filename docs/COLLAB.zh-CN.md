...
- **重进与恢复时的三方合并。** 重新进入一个带缓存的房间（或会话中途链路恢复）时，客户端合并 base（上次同步的场景）/ ours（本地编辑）/ theirs（快照）。单侧改动干净合并；无法解决的冲突——同一元素两侧都改过（改-改、改-删、删-改）——按**在线版本胜出**处理：本地改动被强制重置并给出琥珀色提示（"N 处本地编辑发生冲突——已保留在线版本"，附"显示"高亮）。没有离线编辑的纯缓存则直接被快照覆盖。
